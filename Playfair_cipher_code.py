...
def locindex2(c): #get location of each character
    loc=list()
    if c==EntryKeyFrom2.get().upper():
        c=EntryKeyTo2.get().upper()
    for i ,j in enumerate(my_matrix):
        for k,l in enumerate(j):
            if c==l:
                loc.append(i)
...
def DECRYPT():
    ...
    while i<len(msg):
        ...
        if loc[1]==loc1[1]:
            c+=my_matrix[(loc[0]-1)%5][loc[1]]+my_matrix[(loc1[0]-1)%5][loc1[1]]
        elif loc[0]==loc1[0]:
            c+=my_matrix[loc[0]][(loc[1]-1)%5]+my_matrix[loc1[0]][(loc1[1]-1)%5]  
        else:
            c+=my_matrix[loc[0]][loc1[1]]+my_matrix[loc1[0]][loc[1]]    
        i=i+2        
    cc=list(c)
...
Inoneticx_Logo = tk.Label(top,text="GAURAV SHARMA")
